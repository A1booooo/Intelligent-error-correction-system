...
<<<<<<< HEAD
  ...
  const parseJudgeText = (raw: string): string => {
    if (!raw) return '';
...
<<<<<<< HEAD
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              await judgeQuestionStream({
                                questionId: generatedQuestion.questionId,
                                answer: normalizedAnswer.toUpperCase(),
                                signal: controller.signal,
                                onMessage: (chunk) => {
                                  setJudgeResult((prev) =>
                                    (prev || '') + parseJudgeText(chunk)
                                  );
                                },
                                onError: (error) => {
                                  console.error('判题失败:', error);
                                  setJudgeError(
                                    error?.message || '判题失败，请稍后重试'
                                  );
                                },
                              });
                            } catch (err: any) {
                              console.error('判题失败:', err);
                              setJudgeError(err.message || '判题失败，请稍后重试');
                            } finally {
                              setIsJudging(false);
                            }
                          }}
                          disabled={isJudging || !answerInput.trim()}
                      ...
                      {judgeError && (
                        <div className="mt-2 text-xs text-red-600">{judgeError}</div>
                      )}
                      {judgeResult && !judgeError && (
                        <div className="mt-2 text-xs text-green-600">{judgeResult}</div>
                      )}

                      <div className="pt-2 border-t border-dashed border-slate-200 mt-2 flex items-center justify-between">
                        <div className="text-xs text-slate-500">
                          {recordMessage || '可将本题录入错题库，便于后续巩固练习。'}
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          ...
                          onClick={async () => {
                            ...
                            try {
                              await recordQuestion(generatedQuestion.questionId);
                              setRecordMessage('已成功录入错题库');
                            } catch (e: any) {
                              console.error('录入错题库失败:', e);
                              setRecordMessage(e?.message || '录入错题库失败，请稍后重试');
                            } finally {
                              setIsRecording(false);
                            }
                          }}
                        >
                          {isRecording ? '录入中...' : '录入错题库'}
                        </Button>
                      </div>
                    </div>
