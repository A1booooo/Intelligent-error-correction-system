...
// --- 全屏内页视图 ---
const InternalLearningView = ({
  title,
  description,
  mistakeIds,
  onBack,
}: InternalLearningViewProps) => {
  const [practiceStep, setPracticeStep] = useState<
    'idle' | 'loading' | 'answering' | 'result'
  >('idle');
  const [questionData, setQuestionData] = useState<GenerationData | null>(null);
  const [userAnswer, setUserAnswer] = useState('');
  const [judgeResult, setJudgeResult] = useState<'correct' | 'wrong' | null>(
    null,
  );
  ...
  const handleGenerateQuestion = async () => {
    ...
    try {
      const randomId =
        mistakeIds[Math.floor(Math.random() * mistakeIds.length)];
          ...
          <div className="flex-[1.5] bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden h-full">
            ...
            <div className="flex-1 p-6 overflow-y-auto custom-scroll min-h-0">
              <div className="max-w-4xl mx-auto">
                <h2 className="text-xl font-medium text-slate-800 mb-4 text-center">
                  AI 生成笔记
                </h2>
                  ...
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                      <p className="text-xs text-slate-500 mb-3">
                        基于错题 {mistakeIds[0]} 生成变式题
                      </p>
                      <Button
                        onClick={handleGenerateQuestion}
                        size="sm"
                        className="bg-white border-slate-200 text-blue-600 hover:bg-blue-50 border shadow-sm h-9"
                      >
                ...
                {practiceStep === 'result' && (
                  <div className="flex flex-col h-full items-center justify-center gap-3">
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <span
                        className={`text-base font-bold ${judgeResult === 'correct' ? 'text-green-600' : 'text-red-600'}`}
                      >
    ...
    <div className="flex h-[calc(100vh-65px)] w-full bg-slate-50 overflow-hidden relative font-sans">
      ...
      <div className="absolute top-4 left-4 z-10 flex items-center gap-3">
        <div className="relative w-[280px]">
          ...
          <Input
            placeholder="搜索..."
            className="pl-9 bg-white shadow-sm border-slate-200 rounded-full h-9 text-sm focus-visible:ring-blue-500"
          />
        </div>
      ...
      <div className="flex-1 h-full relative border-r border-slate-200">
        {isLoading ? (
          ...
          >
            ...
            <MiniMap
              className="bg-white border-slate-200 shadow-md rounded-lg m-4"
              nodeColor={(n) => (n.type === 'input' ? '#3b82f6' : '#e2e8f0')}
            />
          </ReactFlow>
        )}
      </div>
                    ...
                    {relatedPoints.length > 0 ? (
                      relatedPoints.map((p) => (
                        <Badge
                          key={p.id}
                          variant="secondary"
                          className="px-2 py-0.5 bg-slate-100 text-slate-600 hover:bg-slate-200 font-normal cursor-pointer text-xs"
                        >
                          {p.keyPoints}
                        </Badge>
                      ))
